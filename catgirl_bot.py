...
class CatgirlBot:
    ...
    Parameters:
        model_path: 合并后的模型路径或 ModelScope ID
        lora_path:  如果不用合并模型，传 LoRA 适配器路径（双模型模式）
        load_in_4bit: 是否 4-bit 量化加载（推荐，省 VRAM）
        headless:    浏览器是否无头模式
    """
    ...
    def __init__(
        ...
        self._model = None
        self._kw_model = None  # separate base model if using lora_path
    ...
    def load(self):
        ...
        if self._lora_path:
            ...
            self._kw_model = self._model  # keep base separately
            self._model = PeftModel.from_pretrained(self._kw_model, self._lora_path)
            self._model.eval()
            print(f"[CatgirlBot] LoRA loaded from {self._lora_path}")
    ...
    def ask(self, question: str) -> str:
        ...
        Args:
            question: 用户问题，如 "鬼灭之刃里水之呼吸的使用者有哪些？"
        ...
        print(f"[CatgirlBot] 搜索完成 ({time.time()-t0:.1f}s, ref={len(ref)}c)")

        # 3. 回答
        answer = self._answer(question, ref)
        print(f"[CatgirlBot] 总耗时: {time.time()-t0:.1f}s")
        return answer
    ...
    KW_SYSTEM = "提取搜索关键词。只输出用顿号分隔的关键词，不要其他内容。"
    KW_GARBAGE = ["user", "问题", "assistant", "system", "question", "参考", "搜索"]

    def _extract_keywords(self, question: str) -> list[str]:
        examples = "\n".join(
            f"问题：{q}\n关键词：{kw}" for q, kw in self.KW_EXAMPLES.items()
        )
        prompt = f"{examples}\n问题：{question}\n关键词："
        raw = self._generate(self.KW_SYSTEM, prompt, max_tokens=30, temp=0.1)
        ...
        for k in keywords:
            ...
            if len(k) < 2 or len(k) > 30:
                continue
            if any(g in k.lower() for g in self.KW_GARBAGE):
                continue
            clean.append(k)
        ...
        print(f"[CatgirlBot] 关键词: {clean[:3]}")
        return clean[:3]
    ...
    def _baidu(self, query: str) -> str:
        try:
            ...
            self._page.goto(f"https://baike.baidu.com/item/{q}", wait_until="commit", timeout=15000)
            ...
            if body:
                lines = [l.strip() for l in body.inner_text().split("\n") if len(l.strip()) > 20]
                if lines:
            ...
            for title, url in zip(data[1][:2], data[3][:2]):
                try:
                    r2 = requests.post(
                        ...
                        data={"action": "query", "titles": title, "prop": "extracts",
                    ...
                    if len(ext) > 600:
                        paras = [p.strip() for p in ext.split("\n") if len(p.strip()) > 10]
    ...
    def _bing(self, query: str) -> str:
        ...
        try:
            q = urllib.parse.quote(query)
            self._page.goto(f"https://cn.bing.com/search?q={q}", timeout=12000)
    ...
    ANSWER_SYSTEM = """你是「提酱」和「灯酱」——两只共用一个QQ号的猫娘翻译顾问。
...
材料里有的引用，材料里没有的诚实说不知道。不准编造数字、日期、人名。"""
    ...
    def _answer(self, question: str, ref: str) -> str:
        ...
        msg = f"{question}\n\n[参考资料]\n{ref}\n[/参考资料]\n\n{instruction}"
        raw = self._generate(self.ANSWER_SYSTEM, msg, max_tokens=500, temp=0.85,
    ...
    def _generate(self, system: str, prompt: str, max_tokens: int = 100,
                  temp: float = 0.1, do_sample: bool = False, top_p: float = 0.5,
...
def ask(question: str, model_path: str = "vovodadakt/Qwen3.5-4B-Catgirl") -> str:
    """一行调用猫娘机器人（首次调用会自动加载模型，约 5 秒）。

    from catgirl_bot import ask
    print(ask("鬼灭之刃里水之呼吸的使用者有哪些？"))
    """
    global _bot
    if _bot is None:
